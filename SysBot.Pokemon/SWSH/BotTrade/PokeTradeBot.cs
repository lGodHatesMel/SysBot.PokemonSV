...
namespace SysBot.Pokemon
{
    public class PokeTradeBot : PokeRoutineExecutor8, ICountBot
    {
        ...
        private async Task<PokeTradeResult> PerformLinkCodeTrade(SAV8SWSH sav, PokeTradeDetail<PK8> poke, CancellationToken token)
        {
            ...
            var TradePartner = await GetTradePartnerInfo(TradeMethod.LinkTrade, token).ConfigureAwait(false);
            TrainerName = TrainerName.Replace('&', '+');
            
            bool IsSafe = poke.Trainer.ID == 0 ? true : NewAntiAbuse.Instance.LogUser(TradePartner.IDHash, TrainerNID, poke.Trainer.ID.ToString(), poke.Trainer.TrainerName, Hub.Config.TradeAbuse.MultiAbuseEchoMention, out _);
            if (!IsSafe)
            {
            ...
            Log($"Found Trading Partner: {TrainerName}-{TradePartner.TID} ({poke.Trainer.TrainerName}) (NID: {TrainerNID})");
            ...
            poke.SendNotification(this, $"Found Trading Partner: {TrainerName} TID: {TradePartner.TID}. Waiting for a Pokémon...");
            ...
            (toSend, update) = await GetEntityToSend(sav, poke, offered, oldEC, toSend, trainer, TradePartner, poke.Type == PokeTradeType.Seed ? itemReq : null, token).ConfigureAwait(false);
        ...
        protected virtual async Task<(PK8 toSend, PokeTradeResult check)> GetEntityToSend(SAV8SWSH sav, PokeTradeDetail<PK8> poke, PK8 offered, byte[] oldEC, PK8 toSend, PartnerDataHolder partnerID, TradePartnerSWSH tp, SpecialTradeType? stt, CancellationToken token)
        {
            return poke.Type switch
            {
                PokeTradeType.Random => await HandleRandomLedy(sav, poke, offered, toSend, partnerID, tp, token).ConfigureAwait(false),
                ...
                PokeTradeType.Seed when stt is SpecialTradeType.WonderCard => await JustInject(sav, offered, tp, poke, token).ConfigureAwait(false),
        ...
        private async Task<(PK8 toSend, PokeTradeResult check)> HandleRandomLedy(SAV8SWSH sav, PokeTradeDetail<PK8> poke, PK8 offered, PK8 toSend, PartnerDataHolder partner, TradePartnerSWSH tp, CancellationToken token)
        {
        ...
        private async Task<(PK8 toSend, PokeTradeResult check)> JustInject(SAV8SWSH sav, PK8 offered, TradePartnerSWSH tradePartner, PokeTradeDetail<PK8> poke, CancellationToken token)
        {
            await Click(A, 0_800, token).ConfigureAwait(false);
...
#pragma warning restore 4014
        ...
        private async Task<TradePartnerSWSH> GetTradePartnerInfo(TradeMethod tradeMethod, CancellationToken token)
        {
            var ofs = GetTrainerNameOffset(tradeMethod);
            var data = await Connection.ReadBytesAsync(ofs, 26, token).ConfigureAwait(false);

            var ofsID = GetTrainerTIDSIDOffset(tradeMethod);
            var dataID = await Connection.ReadBytesAsync(ofsID, 4, token).ConfigureAwait(false);

            return new TradePartnerSWSH(dataID, data);
        }
